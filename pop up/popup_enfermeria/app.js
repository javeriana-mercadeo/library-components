...
function renderModalDiscount(elements) {
  ...
  const investmentDiscount = calculateDiscount(
    investment.textContent,
    discountRateModal.textContent
  );
  console.log(investmentDiscount);

  // RENDERIZAR LOS DATOS EN EL MODAL
  const remainingDaysText =
    remainingDays > 1
      ? `quedan ${remainingDays} días`
      : `queda ${remainingDays} día`;

  dateModals.forEach((item) => (item.textContent = remainingDaysText));
  investmentModals.forEach(
    (item) => (item.textContent = investment.textContent)
  );
  discountModals.forEach(
    (item) =>
      (item.textContent = `$${investmentDiscount.discount.toLocaleString()}*`)
  );
  totalModals.forEach(
    (item) =>
      (item.textContent = `$${investmentDiscount.total.toLocaleString()}*`)
  );
}
