...
// Compilado el: 24/09/2025, 03:32:35 p. m. (COT)
  ...
  var RequirementsSystem = {
    ...
    async renderRequirements(programCode) {
      ...
      try {
        ...
        container.innerHTML = '<div class="overflow-auto portlet-msg-error">No se pudieron cargar los criterios de evaluaci\xF3n para este programa. Verifique su conexi\xF3n e intente nuevamente.</div>';
        return false;
      }
    },
    setupEventListeners() {
        ...
        mutations.forEach((mutation) => {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              const container = node.querySelector?.(this.config.containerSelector) || (node.matches?.(this.config.containerSelector) ? node : null);
