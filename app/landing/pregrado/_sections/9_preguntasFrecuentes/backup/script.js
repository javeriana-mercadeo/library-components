...
// Compilado el: 08/10/2025, 04:26:00 p. m. (COT)
...
;(() => {
  ...
  var RequirementsSystem = {
    ...
    processRequirementsData(data) {
      console.log('\u{1F50D} DEBUG: processRequirementsData - data recibida:', data)
      ...
      console.log('\u{1F50D} DEBUG: programInfo:', programInfo)
      const categoriesMap = /* @__PURE__ */ new Map()
      data.forEach(item => {
        console.log('\u{1F50D} DEBUG: procesando item:', {
        ...
        console.log('\u{1F50D} DEBUG: criterio agregado:', criterionObj)
        categoriesMap.get(category).criteria.push(criterionObj)
      })
      const categories = Array.from(categoriesMap.values())
      console.log('\u{1F50D} DEBUG: categories procesadas:', categories)
      return { categories, programInfo }
    },
    generateRequirementsHTML(processedData) {
      console.log('\u{1F50D} DEBUG: generateRequirementsHTML - processedData:', processedData)
      ...
      let html = `
        <div class="requirements-table-wrapper">
          <table>
            <caption>Criterios de Evaluaci\xF3n - ${programInfo.name}</caption>
            <thead>
              <tr>
      ...
      categories.forEach(category => {
        ...
        category.criteria.forEach((criterion, index) => {
          console.log('\u{1F50D} DEBUG: generando HTML para criterio:', {
            name: criterion.name,
    ...
    async renderRequirements(programCode) {
      ...
      container.innerHTML = '<p>Cargando criterios de evaluaci\xF3n...</p>'
      try {
        ...
        container.innerHTML =
          '<div class="overflow-auto portlet-msg-error">No se pudieron cargar los criterios de evaluaci\xF3n para este programa. Verifique su conexi\xF3n e intente nuevamente.</div>'
        return false
      }
    },
    setupEventListeners() {
