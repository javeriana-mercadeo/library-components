...
class VideoSystem {
   ...
   */
  initIOSStrategy() {
    // Usar estrategia progresiva mejorada para iOS
  ...
  createVideoElement(container, videoUrl, fallbackImage) {
    ...
    Object.assign(video, {
      ...
      controls: false,
      autoplay: false, // Explícitamente false para evitar bloqueos
      crossorigin: 'anonymous'
    })

    // Añadir atributos como propiedades del DOM para mejor compatibilidad
    ...
    this.events.addEventListener(video, 'loadedmetadata', () => {
      // Video metadata cargada - ya podemos mostrar el primer frame
      video.style.opacity = '0.5'
    })

    this.events.addEventListener(video, 'canplay', () => {
      // Video listo para reproducir - mostrar completamente
      video.style.opacity = '1'
      this.state.loadedVideos.add(video)
    })

    this.events.addEventListener(video, 'loadeddata', () => {
      // Datos del video cargados - mostrar el video
      video.style.opacity = '1'
    })
    ...
    this.events.addEventListener(video, 'stalled', () => {
      // Video se atascó - mostrar loading
      const loading = this.showMinimalLoading(container)
      loading.style.zIndex = '3'
    })

    this.events.addEventListener(video, 'waiting', () => {
      // Video esperando datos - mostrar loading temporal
      const loading = this.showMinimalLoading(container)
      loading.style.zIndex = '3'
    })

    this.events.addEventListener(video, 'playing', () => {
      // Video reproduciéndose - ocultar loading
