...
// Compilado el: 25/08/2025, 11:39:20 a. m. (COT)
      ...
      const initializeWithStrategy = (strategy) => {
        ...
        const tryInit = () => {
          ...
          if (containers.length > 0) {
            ...
            console.warn(`[VideoPlayer] Estrategia ${strategy.name}: no se encontraron contenedores despu\xE9s de ${strategy.maxAttempts} intentos`);
            strategyIndex++;
            if (strategyIndex < initStrategies.length && !globalInitialized) {
              setTimeout(() => initializeWithStrategy(initStrategies[strategyIndex]), 200);
            }
