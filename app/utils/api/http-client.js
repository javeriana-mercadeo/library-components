...
class HTTPClient {
  constructor(baseURL = '', options = {}) {
    ...
    this.logger = (typeof window !== 'undefined' && window.Logger) || {
      debug: () => {},
      error: () => {},
      success: () => {},
      info: () => {}
    ...
    this.processRequestInterceptors = async function (config) {
      ...
      for (const interceptor of this.requestInterceptors) {
        try {
          ...
          this.logger.error?.('Error en request interceptor', error)
        }
      }
    ...
    this.processResponseInterceptors = async function (response, config) {
      ...
      for (const interceptor of this.responseInterceptors) {
        try {
          ...
          this.logger.error?.('Error en response interceptor', error)
        }
      }
    ...
    this.processErrorInterceptors = async function (error, config) {
      ...
      for (const interceptor of this.errorInterceptors) {
        try {
          ...
          this.logger.error?.('Error en error interceptor', interceptorError)
        }
      }
    ...
    this.makeRequest = async function (method, endpoint, options = {}) {
      ...
      const makeAttempt = async () => {
        ...
        try {
          ...
          this.logger.debug?.(`${processedConfig.method} request iniciado`, { url: processedConfig.url })
          ...
          this.logger.success?.(`${processedConfig.method} request exitoso`, { url: processedConfig.url })
          ...
          this.logger.error?.(`Error en ${processedConfig.method} request`, { url: processedConfig.url, error })
    ...
    this.createCancelToken = function () {
      ...
      return {
        ...
        cancel: (reason = 'Request cancelled') => {
          controller.abort()
          this.logger.info?.('Request cancelado', { reason })
        }
      }
    }
