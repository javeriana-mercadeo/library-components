...
// Compilado el: 18/08/2025, 07:20:08 p. m. (COT)
...
(() => {
  ...
  // app/_library/components/header/components/modalFrom.js
  var ModalForm = {
      ...
      document.addEventListener("data_load-program", async (event) => {
        try {
          ...
          if (typeof Logger !== "undefined" && Logger.info) {
            Logger.info(`\u{1F4CB} [FORM] Iniciando FormModules para: ${programa} (${codPrograma})`);
          } else {
            console.log(`Iniciando FormModules para el programa: ${programa} con c\xF3digo: ${codPrograma}`);
          }
          ...
          if (typeof Logger !== "undefined" && Logger.success) {
            Logger.success(`\u{1F4CB} [FORM] FormModules inicializado correctamente para ${programa}`);
          }
        } catch (error) {
          if (typeof Logger !== "undefined" && Logger.error) {
            Logger.error("\u{1F4CB} [FORM] Error inicializando FormModules:", error);
          } else {
